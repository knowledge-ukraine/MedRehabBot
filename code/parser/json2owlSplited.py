...
def get_classes(p_dict, p_key, output, title, uncertain, counter):
    def make_individual(item, title, key, output, counter):
        ...
        if len(new_entity_id) > 0:
            ...
            if new_entity_id not in output["entities"]:
                new_individual = OntoEntity(id="n_ind"+str(len(output["entities"])+1),  # new_entity_id,
                                            ...
                                            label=" ".join(new_name.split()[:7]).replace(" & ", " and ").replace(
                                                           ...
                                                           ">", "more than"),
                                            language="en",
                                            assertions={"relate_to_article": ["n_ind"+title_id]})

                counter += 1
                # print(len(output["entities"])+1)
                ...
                if title_id not in output["entities"][new_entity_id].assertions["relate_to_article"]:
                    output["entities"][new_entity_id].assertions["relate_to_article"].append("n_ind"+title_id)
    ...
    if title not in output["classes"]:
        ...
        new_title_individual = OntoEntity(id="n_ind"+new_title_entity_id, of_classes="articles_names",
                                          assertions={"relate_to_article": ["n_ind"+new_title_entity_id]},
        ...
        if not (p_key in uncertain and len([j for j in p_dict[key] if
            ...
            if new_class_id not in output["classes"]:
                new_class = OntoClass(id="cl_"+new_class_id,
                                      ...
                                      label=' '.join(key.strip().strip(",").strip(";").strip(":").strip(
                                          ".").split()[:7]).replace(" & ", " and ").replace(
                                          "& ", "and ").replace(
                ...
                if len(word) > 1 and (word[1] == "NN" or word[1] == "NNS" or word[1] == 'VBG' or
                    ...
                    if no_djts:
                        if word[1] not in word_classes:
                            new_class = OntoClass(id=word[1], parent_class_id="words", label=word[1].strip().strip(
                                ",").strip(";").strip(":").strip("."),
                                                  language="en")
...
if __name__ == "__main__":
    ...
    all_words = set()
    ch_len = 2110
    input_files_names_ch = list()
    chunk = list()
    for n, name in enumerate(input_files_names):
        chunk.append(name)
        if n % ch_len == 0 and n > 0:
            input_files_names_ch.append(copy.deepcopy(chunk))
            chunk = list()
    if len(chunk) > 0:
        input_files_names_ch.append(copy.deepcopy(chunk))
    for n, chunk in enumerate(input_files_names_ch):
        print("Chunk: ", n+1, len(chunk))
        ...
        for t in terms:
            if isinstance(terms[t].label, str):
                all_words.add(terms[t].label)
            else:
                for w in terms[t].label:
                    all_words.add(w)

        onto_objects["entities"].update(terms)
        for t in onto_objects["classes"]:
            if isinstance(onto_objects["classes"][t].label, str):
                tokenized_text = nltk.pos_tag(nltk.tokenize.word_tokenize(onto_objects["classes"][t].label))
                for word in tokenized_text:
                    if len(word) > 1:
                        lemma_word = lemmatizer.lemmatize(word[0].strip().lower())
                        all_words.add(lemma_word)
            else:
                for label in onto_objects["classes"][t].label:
                    tokenized_text = nltk.pos_tag(nltk.tokenize.word_tokenize(label))
                    for word in tokenized_text:
                        if len(word) > 1:
                            lemma_word = lemmatizer.lemmatize(word[0].strip().lower())
                            all_words.add(lemma_word)
        ...
        tau_0 = time.time()
        save_owl("owl/ContextOntologyCommon_" + str(n+1) + ".owl", text_owl=owl_text)
        print(time.time() - tau_0, " sec")
        print("Process is complete")
        print()
    print(len(all_words), " words found")
    f = open("owl/all_words.json", "w", encoding="utf-8")
    f.write(json.dumps(list(all_words), indent=2, ensure_ascii=False, sort_keys=True))
    f.close()
